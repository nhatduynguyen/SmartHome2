...
var accAddr01Initialized = false;
        ...
        req.on('end', function () {
			...
			if(!rcInstance) {
				if (typeof web3 !== 'undefined') {
				    ...
				    console.log(formatted+' [server-sendRequest] currentProvider is initialized');
			    } else {
			        blockchainProvider = new Web3.providers.HttpProvider(blockchainUri);
			        web3 = new Web3(blockchainProvider);
			        console.log(formatted+' '+blockchainUri+' is initialized');
			        rcInstance = web3.eth.contract(rcAbi).at(rcAddr);
			...
			for (i = 0; i < contractNumbers.length; i++) {
			    ...
			    try {
					...
					console.log(formatted+' '+'Listening event on ACC: '+accAddr);
			...
			try {
				// ACC 01
				var accAddr01 = rcInstance.getContractAddr(methodName01);
				var accAbiBytes01 = rcInstance.getContractAbi(methodName01);

				var accInstance01 = web3.eth.contract(JSON.parse(web3.toAscii(accAbiBytes01))).at(accAddr01);
				var accEvent01 = accInstance01.ReturnAccessResult({fromBlock: 0, toBlock: 'latest'});
				accAddr01Initialized = true;
			} catch (err) {
				console.error(err);
				accAddr01Initialized = false;
			}

			if(accAddr01Initialized) {
				accEvent01.watch(function(err, result){
					if(!err) {
						var msg = '';
						msg += "Contract: " + result.address+"\n";
						msg += "Block Number: " + result.blockNumber+"\n";
			...
			try {
				// ACC 02
				var accAddr02 = rcInstance.getContractAddr(methodName02);
				var accAbiBytes02 = rcInstance.getContractAbi(methodName02);

				var accInstance02 = web3.eth.contract(JSON.parse(web3.toAscii(accAbiBytes02))).at(accAddr02);
				var accEvent02 = accInstance02.ReturnAccessResult({fromBlock: 0, toBlock: 'latest'});
				accAddr02Initialized = true;
			} catch (err) {
				console.error(err);
				accAddr02Initialized = false;
			}

			if(accAddr02Initialized) {
				accEvent02.watch(function(err, result){
					if(!err) {
						var msg = '';
						msg += "Contract: " + result.address+"\n";
						msg += "Block Number: " + result.blockNumber+"\n";
						...
						msg += "\n";
						console.log(formatted+' [EVENT]:\n'+msg);
						io.emit('event_provider', msg, result.args._from);
					}
				})
			}
			*/
